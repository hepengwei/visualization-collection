...
const OPENING_HEIGHT = DOOR_HEIGHT + TOP_GAP;
const doorConfigs = [
  // 主卧门
  {
...
const createDoor = (
  ...
  let doorPanelGroupPositionX = -HALF_DOOR_WIDTH;
  ...
  let handleGroupPositionX = DOOR_WIDTH - 0.08;
  if (handlePosition === "left") {
  ...
  // 安装三个合页（上、中、下）
  const hingeYs = [DOOR_HEIGHT * 0.82, DOOR_HEIGHT * 0.5, DOOR_HEIGHT * 0.18];
...
const createButtHinge = (
  ...
  // 门扇侧 leaf（右半，贴在门扇左侧面）
  const leafT = HINGE_WIDTH / 2; // 单个叶板厚度（X方向，夹在缝隙中）

  // 门扇侧 leaf — 局部坐标：x = -HALF_DOOR_WIDTH（门扇左边缘）
  let leaf1PositonX = -HALF_DOOR_WIDTH - leafT / 2;
  if (handlePosition === "left") {
    leaf1PositonX = -leaf1PositonX;
  }

  const leaf1 = new Mesh(boxGeometry, hingeMaterial);
  leaf1.scale.set(leafT, HINGE_HEIGHT, HINGE_THICKNESS);
  ...
  // 门框侧 leaf
  let leaf2PositonX = -HALF_DOOR_WIDTH + leafT / 2;
  if (handlePosition === "left") {
    leaf2PositonX = -leaf2PositonX;
  }
  const leaf2 = new Mesh(boxGeometry, hingeMaterial);
  leaf2.scale.set(leafT, HINGE_HEIGHT, HINGE_THICKNESS);
